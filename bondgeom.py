...
def fit_poly_geom():
    ...
    cn = poly_geom['cn']
    nndist = poly_geom['nndist']
    nnpairnum = poly_geom['nnpairnum']
    bondang = poly_geom['bondang']

    # Give equal weighting to all points
    wt = np.ones(nndist.shape)
    # wt[cn==2]  = 100
    # wt[cn==4]  = 3
    # wt[cn==6]  = 3
    # wt[cn==12] = 3

    # Do unweighted polynomial fit of nnpairnum
    coef = np.polyfit(cn,nnpairnum,2)

    def resid_f( param, cn=cn, nndist=nndist, wt=wt):
        resid = wt*(nndist - decay_model(param,cn))
        return resid
    ...
    nndist_mod = decay_model( paramf, cnmod )
    bondang_mod = calc_bond_ang( nndist_mod )

    # plt.clf()
    # plt.rc('text', usetex=True)
    # ideal_msk = np.in1d(cn,[4,6,12])
    # plt.plot(cn[~ideal_msk],edgelen[~ideal_msk],'ko',cn[ideal_msk],edgelen[ideal_msk],'kx',cnmod,edgelen_mod,'r-',mew=2)
    # plt.xlim(1.9,16.3)
    # plt.ylim(.75,2.05)
    # plt.plot(cn[ind_ideal],edgelen,'ko',cnmod,edgelen_mod,'r-')
    # plt.xlabel('Coordination Number')
    # plt.ylabel('Oxygen Dist.  /  Cat-Oxy Bond Length')
    # plt.ylabel(r'$d_{\textrm{oxy-oxy}} \;  / \; d_{\textrm{cat-oxy}}$')
    # plt.savefig('test/coordination-bonding-geom.png',dpi=400)
    # plt.plot(cn,bondang,'ko',cnmod,bondang_mod,'r-')

    def nndist_model( cn, param=paramf ):
        return decay_model(param, cn)

    poly_geom['nndist_param'] = paramf
    poly_geom['nndist_f'] = lambda cn, param=paramf: decay_model(param,cn)
    poly_geom['nnpairnum_coef'] = coef
    poly_geom['nnpairnum_f'] = lambda cn, coef=coef: np.polyval(coef,cn)

    nndist_inv_f = interp.interp1d(nndist_mod,cnmod)
    poly_geom['nndist_inv_f'] = nndist_inv_f

    return poly_geom
#====================================================================
def get_poly_geom():
    # Determine NN distances for circles spread on surface of sphere
    # From Clare1991
    # TODO: replace this calculation
    # cn_a = np.arange(2,30.1)
    # nndist_a = np.array([2.0, 1.732051, 1.632993, 1.414214, 1.414214, 1.256870,
    #                      1.215563, 1.154701, 1.091426, 1.051462, 1.051462, 0.956414,
    #                      0.933863, 0.902656, 0.880574, 0.862445, 0.838217, 0.808558,
    #                      0.804392, 0.775239, 0.761175, 0.744517, 0.744206, 0.710776,
    #                      0.700983, 0.695141, 0.672110, 0.662797, 0.660981])

    poly_geom = {}

    poly_pos = get_poly_pos()

    cn_a = np.array( poly_pos.keys() )
    bondang_a = []
    nndist_a = []
    nnpairnum_a = []


    for cnkey in poly_pos:
        # Get current oxygen bonded atom positions
        pos_a = poly_pos[cnkey]
        distij_a = calc_pair_dist_matrix( pos_a, mask_self=True )

        neigh_dist_a = distij_a[0]
        nncoor_eff_a = np.zeros(cnkey)
        nndist_eff_a = np.zeros(cnkey)
        for ind,neigh_dist_a in enumerate(distij_a):
            nncoor_eff, nndist_eff = calc_eff_coor( neigh_dist_a )
            nncoor_eff_a[ind] = nncoor_eff
            nndist_eff_a[ind] = nndist_eff

        nndist_avg = np.mean(nndist_eff_a)
        bondang_avg = 2*np.arcsin(0.5*nndist_avg)*180/np.pi
        nnpairnum = 0.5*np.sum(nncoor_eff_a)

        # print np.sum(poly_pos[cnkey]**2,axis=1)
        # idistNN = np.mean(np.nanmin(distij_a,axis=0))
        # idistNNmin = np.nanmin(distij_a)
        # ibondang = 2*np.arcsin(idistNN/2.0)*180/np.pi
        # edgelenmin.append(idistNNmin)
        nndist_a.append(nndist_avg)
        bondang_a.append(bondang_avg)
        nnpairnum_a.append(nnpairnum)

    bondang_a = np.array( bondang_a )
    nnpairnum_a = np.array( nnpairnum_a )
    nndist_a = np.array( nndist_a )
    # edgelenmin = np.array( edgelenmin )

    poly_geom['cn'] = cn_a
    poly_geom['pos'] = poly_pos
    poly_geom['bondang'] = bondang_a
    poly_geom['nndist'] = nndist_a
    poly_geom['nnpairnum'] = nnpairnum_a
    # poly_geom['edgelenmin'] = edgelenmin
...
#====================================================================
#  General Atomic Geometry Calculations
#====================================================================
def calc_closest_packing_nndist( V, nobj ):
    ...
    return nndist
#====================================================================
def calc_bond_ang( nndist, bondlen=1.0 ):
    ratio = nndist/bondlen
    bondang = 2*np.arcsin(ratio/2.0)*180/np.pi
    return bondang
#====================================================================
def resamp_model( x_a, y_a, N=3001 ):
    xmod_a = np.linspace(x_a[0],x_a[-1],N)
    ymod_a = interp.interp1d(x_a,y_a)(xmod_a)

    return xmod_a, ymod_a
#====================================================================
def extract_nnshell( r_a, pdf_a, TOL=1e-9 ):
    indmax = np.argmax(pdf_a)
    indmin1 = np.argmin( pdf_a[indmax:] ) +indmax + 1

    pkmax = pdf_a[indmax]
    delind = np.minimum(np.round(0.5*(indmin1-indmax)).astype(np.int), 3)

    def calc_minmax( r, pdf ):
        spl_ext = interp.splrep(r, pdf,k=4)
        spl_deriv = interp.splder( spl_ext )
        rext = interp.sproot(spl_deriv)[0]
        pdf_ext = interp.splev(rext, spl_ext)
        return rext, pdf_ext

    rmin1,pdfmin1 = calc_minmax( r_a[indmin1-delind:indmin1+delind],
                                pdf_a[indmin1-delind:indmin1+delind] )

    indnn = np.where(np.abs(pdf_a)>TOL*pkmax)[0][0]
    rnn = r_a[indnn]

    r_nnshell_a = np.append(r_a[indnn:indmin1],rmin1)
    pdf_nnshell_a = np.append(pdf_a[indnn:indmin1],pdfmin1)

    return r_nnshell_a, pdf_nnshell_a
#====================================================================
def calc_pdf_geom( rho, r_a, pdf_a ):
    # rho = N/V

    # Extract 1st nearest neighbor shell
    r_nnshell_a, pdf_nnshell_a = extract_nnshell( r_a, pdf_a )
    rnn = r_nnshell_a[0]
    rmin1 = r_nnshell_a[-1]

    rmod_a, pdfmod_a = resamp_model( r_nnshell_a, pdf_nnshell_a )

    # coordination wt = # of atoms in each shell
    cnshell_a = 4*np.pi*rho*rmod_a**2*pdfmod_a

    cn = np.trapz(cnshell_a,x=rmod_a)
    avg_nndist = np.trapz(cnshell_a*rmod_a,x=rmod_a)/cn


    # Also calculate properties using Hoppe1979 weighted Avg scheme

    N = 3001
    rmod_full_a = np.linspace( rnn, r_a[-1], N )
    pdfmod_full_a = interp.interp1d( r_a, pdf_a)(rmod_full_a)

    dr = rmod_full_a[1]-rmod_full_a[0]
    mult_a = 4*np.pi*rho*rmod_full_a**2*pdfmod_full_a*dr

    cn_eff, nndist_eff = calc_eff_coor( rmod_full_a, mult_a=mult_a, TOL=1e-7)

    # plt.clf()
    # plt.plot(rmod_full_a, pdfmod_full_a, 'rx', rmod_a, pdfmod_a,'k--',r_a,pdf_a,'go')


    fpack_nnavg = rho*np.pi/6*avg_nndist**3

    output_d = {}
    output_d['rho'] = rho
    output_d['r_a'] = r_a
    output_d['pdf_a'] = pdf_a
    output_d['r_nnshell_a'] = rmod_a
    output_d['pdf_nnshell_a'] = pdfmod_a
    output_d['r_nn'] = rnn
    output_d['r_nnavg'] = avg_nndist
    output_d['r_nneff'] = nndist_eff
    output_d['r_min1'] = rmin1
    output_d['cn'] = cn
    output_d['cneff'] = cn_eff
    output_d['fpack_nnavg'] = fpack_nnavg

    return output_d
#====================================================================
def calc_pair_dist_matrix( pos_a, mask_self=True ):
    ipos = np.expand_dims(pos_a,axis=0)
    jpos = np.expand_dims(pos_a,axis=1)
    distij_a = np.sqrt(np.sum((ipos-jpos)**2,axis=2))

    if mask_self:
        np.fill_diagonal(distij_a,np.nan)

    return distij_a
#====================================================================
def calc_eff_coor( dist_a, mult_a=None, TOL=1e-4 ):

    # If unset, multiplicity is 1 (one atom at each distance)
    if mult_a is None:
        mult_a = np.ones( dist_a.shape )

    # screen out NaNs
    nan_mask_a = ~np.isnan(dist_a)
    dist_a, mult_a = dist_a[nan_mask_a], mult_a[nan_mask_a]

    # sort distances (and remove any NaNs)
    indsort_a = np.argsort( dist_a )

    dists_a = dist_a[indsort_a]
    mults_a = mult_a[indsort_a]

    nndist_eff = dists_a[0]
    # print nndist_eff

    while True:
        wt_a = mults_a*np.exp( 1 - (dists_a/nndist_eff)**6 )
        cn_eff = np.sum( wt_a )
        nndist_prev = nndist_eff
        nndist_eff = np.sum( dists_a*wt_a/cn_eff )
        # print nndist_eff

        if np.abs(nndist_eff/nndist_prev - 1.0) < TOL:
            break

    return cn_eff, nndist_eff
#====================================================================
